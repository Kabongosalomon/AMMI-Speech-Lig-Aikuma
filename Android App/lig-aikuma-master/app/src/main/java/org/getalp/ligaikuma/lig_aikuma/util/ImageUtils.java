...
public final class ImageUtils {
	 ...
	 */
	public static Bitmap resizeBitmap(Bitmap original, float scale) {
		if (original == null)
			throw new IllegalArgumentException("Can not resize null bitmap");
		if (scale < 0)
			throw new IllegalArgumentException("scale argument cannot be less than zero");
		int width = original.getWidth(), height = original.getHeight();
		Matrix matrix = new Matrix();
		matrix.postScale(scale, scale);
		Bitmap resized = Bitmap.createBitmap(original, 0, 0, width, height, matrix, false);
		assert resized != null;
		return resized;
	}
	...
	public static void enableImageSync(UUID uuid) throws IOException {
		FileUtils.moveFileToDirectory(getNoSyncSmallImageFile(uuid), ImageUtils.getImagesPath(), false);
		FileUtils.moveFileToDirectory(getNoSyncImageFile(uuid), ImageUtils.getImagesPath(), false);
	}
	...
	public static Bitmap getImage(UUID uuid) throws IOException {
		return retrieveFromFile(getImageFile(uuid));
	}
	...
	public static Bitmap getSmallImage(UUID uuid) throws IOException {
		return retrieveFromFile(getSmallImageFile(uuid));
	}
	...
	public static Bitmap getNoSyncSmallImage(UUID uuid) throws IOException {
		return retrieveFromFile(getNoSyncSmallImageFile(uuid));
	}
	...
	public static void moveImageFileFromUri(Context context, 
		...
		InputStream fis = context.getContentResolver().openInputStream(srcUri);
