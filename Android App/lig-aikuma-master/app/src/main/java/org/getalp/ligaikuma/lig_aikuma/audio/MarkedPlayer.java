...
public class MarkedPlayer extends SimplePlayer
{
	...
	public MarkedPlayer(File recordingFile, long sampleRate, boolean playThroughSpeaker) throws IOException {
	...
	public MarkedPlayer(SimplePlayer player, boolean playThroughSpeaker) throws IOException {
		...
		public void run() {
			while (true) {
				...
				if (notificationMarkerPosition >= 0)
					if (getCurrentMsec() >=getNotificationMarkerPositionMsec()) {
						unsetNotificationMarkerPosition();
						onMarkerReachedListener.onMarkerReached(MarkedPlayer.this);
					}
			}
		}
	}
