...
import org.getalp.ligaikuma.lig_aikuma.audio.record.AudioHandler;
import org.getalp.ligaikuma.lig_aikuma.audio.record.recognizers.AverageRecognizer;
import org.getalp.ligaikuma.lig_aikuma.audio.record.recognizers.Recognizer;
...
public class ThresholdSpeechAnalyzer extends Analyzer {
   ...
   */
	...
	protected boolean doesTriggerSilence(short[] buffer) {
		if(recognizer.isSilence(buffer))	silenceTriggers++;
		else								silenceTriggers = 0;
	...
	protected boolean doesTriggerSpeech(short[] buffer) {
		if(recognizer.isSpeech(buffer))	speechTriggers++;
		else							speechTriggers = 0;
	...
	protected void shiftOnsetBufferWith(short[] buffer) {
		onsetBuffer = Arrays.copyOfRange(onsetBuffer, buffer.length, onsetBuffer.length);
	...
	public void analyze(AudioHandler handler, short[] buffer) {
		...
		if (speech) {
			...
			if (doesTriggerSpeech(buffer)) {
				...
				// Empty preamble.
				handler.audioTriggered(new short[onsetBuffer.length * EMPTY_SPEECH_PREAMBLE_BUFFERS], true);
